...
import { createRssSource } from 'utils'
import { Tag } from '@prisma/client'
import { SingleTag } from '../tags'

export const AddFeedForm = ({ setStep }: { setStep: (step: 1 | 2) => void }) => {
              ...
              search.map((tag) => {
                return (
                  <div
                    ...
                    onClick={() => {
                      const input = document.getElementById('tags_search_field') as HTMLInputElement
                      const existingTags = form.getValues('tags')

                      if (existingTags.length < 3 && !existingTags.includes(tag.label))
                        form.setValue('tags', [...form.getValues('tags'), tag.label])
                      setIsSearching(false)
                      input.value = ''
                    }}
                  >
